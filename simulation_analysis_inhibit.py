...
ra_vals = [15, 45, 75, 105, 135, 165, 195, 225, 255, 285, 315, 345, 375, 405]  # Ohm-cm
...
for ra in ra_vals:
    print(f"Running simulation for Ra = {ra} Ohm-cm")
    ...
    neuron.hoc.execute(f"ra={ra}")

    # Update the direction in the hoc file
    neuron.hoc.execute("update_ra()")
    current_direc = 1
...
plt.errorbar(ra_vals, mean_DSI_values, yerr=std_DSI_values, fmt='o', label='Mean DSI ± Std', capsize=5)

plt.xlabel('Ra Values (Ohm-cm)')
plt.ylabel('DSI')
plt.title('Mean DSI ± Standard Deviation for Different Ra Values (With Inhibitory inputs)')
plt.grid(True)
plt.legend()
...
max_dsi_ra = ra_vals[max_dsi_index]
plt.annotate(f'Max DSI: {max_dsi_value:.2f} at Ra={max_dsi_ra}',
             xy=(max_dsi_ra, max_dsi_value),
             xytext=(max_dsi_ra , max_dsi_value -0.1),
...
plt.figure(figsize=(10, 6))
plt.plot(ra_vals, mutual_info_values, marker='o', label='Mutual Information', color='orange')
plt.xlabel('Ra Values (Ohm-cm)')
plt.ylabel('Mutual Information')
plt.title('Mutual Information for Different Ra Values (For Inhibitory inputs)')
plt.grid(True)
plt.legend()
...
max_mi_ra = ra_vals[max_mi_index]

plt.annotate(f'Max MI: {max_mi_value:.2f} at Ra={max_mi_ra}',
                xy=(max_mi_ra, max_mi_value),
                xytext=(max_mi_ra + 10, max_mi_value -0.05),
                arrowprops=dict(facecolor='black', arrowstyle='->'),
                fontsize=10)
